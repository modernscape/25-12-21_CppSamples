...
const size_t kBufferSize = 190;  // 80
...
// ターミナルの描画に使う最大行数
const size_t kTerminalHeight = 10;

void draw(const array<double, kBufferSize>& heightField) {
    // 画面クリア（既存の画面クリア部分を流用するか、より強力な方法を使う）
    // ...

    for (int i = kTerminalHeight - 1; i >= 0; --i) {  // 上の行から下の行へ
        string line(kBufferSize, ' ');
        // 現在の行が担当する高さの閾値を計算
        double threshold = (i + 1.0) / kTerminalHeight;

        for (size_t j = 0; j < kBufferSize; ++j) {
            // heightFieldの値がこの行の閾値を超えていれば文字を配置
            if (heightField[j] >= threshold) {
                // 最も明るい文字を使うなど、適当な文字を選ぶ
                line[j] = '#';
            }
        }
        printf("%s\n", line.c_str());  // 1行出力して改行
...
int main() {
    ...
    const double maxHeightX = 0.5;  // 0.5
    const double maxHeightY = 0.4;  // 0.4
